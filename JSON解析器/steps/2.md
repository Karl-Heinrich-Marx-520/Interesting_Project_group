...
class Lexer {
private:
    const std::string_view m_input;  // 输入的JSON字符串(view本身就是只读， 我们加个const显得严谨)
    std::string m_own_input;     // 右值持有，再赋给view，解决view无法接收右值问题
...
// 接下来用view优化，下面代码就全部注释掉了
//    Lexer(const std::string& input): m_input(input), m_pos(0), m_line(1), m_column(1) {}
//    Lexer(const std::string&& input) : m_input(std::move(input)), m_pos(0), m_line(1), m_column(1) {} //parse那有讲解

    explicit Lexer(std::string_view input) : m_input(input), m_pos(0), m_line(1), m_column(1) {}
    explicit Lexer(std::string&& input) m_own_input(std::move(input)), m_input(m_own_input), m_pos(0), m_line(1), m_column(1) {}
