...
#include <vector>
#include <string_view> //后面Lexer和Parser做一些优化，节省两次深拷贝，数据量几兆时可以显著提高性能
...
public:
    ...
    // 右值引用 移动构造 （AI没写万能引用，模板分情况有点复杂了，我也嫌麻烦就只加个右值构造了，哈哈）
    // 要真上万能引用模板，得做类型限制、偏特化、SFINAE 屏蔽  int / bool / double / nullptr_t  太麻烦了
    ...
    // 给可修改的版本，方便后续赋值
    //一般工业级的代码也会提供其他四种类型的非const版本，这并没啥好讲之处，只是为了照顾一些奇葩用法而已
    //所以我们只提供Array和Object的非const版本，支持整体覆盖修改就行了
    JsonArray& as_array() {
        if (!is_array()) throw std::runtime_error("当前值不是array类型，无法转换");
        return std::get<JsonArray>(m_value);
    }
