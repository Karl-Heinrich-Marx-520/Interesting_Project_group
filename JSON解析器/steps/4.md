...
// indent: 缩进空格数(0=压缩,4=格式化)  current_indent: 当前递归层级
	std::string serialize(int indent = 0, int current_indent = 0) const {
		...
		//匹配variant类型
	// 每种类型单独实例化一份函数，各自裁剪
	// 当时我以为这一段 lambda 是同一份函数，所有类型共用它，constexpr编译期裁一次分支，后面别的类型就没法用了。
	// 但实际上这个带  auto  的 lambda 本身就是模板， std::visit  会给每一种 variant 类型，单独生成一份专属函数副本，
	// 每份副本自己裁剪自己的分支，互相完全不干扰
	// C++ 处理 std::variant 标准写法： std::visit + auto 模板 lambda + if constexpr 类型分发
